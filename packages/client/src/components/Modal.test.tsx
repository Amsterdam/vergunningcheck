...
it("Modal Button should render, but the Modal itself not", () => {
  const { getByText, queryByTestId } = render(
    <Modal heading="test" openButtonText="button">
      text
    </Modal>
  );
...
it("Modal should open and close", async () => {
  ...
  act(() => {
    fireEvent.click(queryByTestId(MODAL_CLOSE_BUTTON) as HTMLElement);
  });
  // Modal = closed
  ...
  act(() => {
    fireEvent.click(queryByTestId(MODAL_DECLINE_BUTTON) as HTMLElement);
  });
  // Modal = closed
